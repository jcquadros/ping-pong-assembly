...
fill_rectangle:
		...
		MOV CX, [BP+10]           ; Load x2 (end X)
		MOV AX, [BP+8]            ; Load x1 (start X)
		MOV DX, [BP+6]            ; Load y1 (start Y)
		MOV BX, [BP+4]            ; Load y2 (end Y)

fill_loop_y:                  ; Outer loop for Y
   	 	MOV SI, AX                ; Reset SI to start X
fill_loop_x:                  ; Inner loop for X
		PUSH DX                   ; Push current Y
		PUSH SI                   ; Push current X
		CALL plot_xy              ; Plot the pixel
		INC SI                    ; Move to the next X
		CMP SI, CX                ; Check if we reached end X
		JBE fill_loop_x           ; If not, continue inner loop

		INC DX                    ; Move to the next Y
		CMP DX, BX                ; Check if we reached end Y
		JBE fill_loop_y           ; If not, continue outer loop
